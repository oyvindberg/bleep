...
object GenerateForTapirGrpcExamples extends BleepCodegenScript("GenerateForTapirGrpcExamples") {
  override def run(started: Started, commands: Commands, targets: List[Target], args: List[String]): Unit = {
    ...
    targets.foreach { target =>
      if (Set("tapir-grpc-examples@jvm212", "tapir-grpc-examples@jvm213").contains(target.project.value)) {
    ...
    targets.foreach { target =>
      ...
      |     *
      |     * Use `Libr""".stripMargin) + new String(s"""|aryHandler.apply` if the server is only handling one service.
      |     *
      |     * Registering a gRPC service under a custom prefix is not widely supported and strongly discouraged by the specification.
      |     */
