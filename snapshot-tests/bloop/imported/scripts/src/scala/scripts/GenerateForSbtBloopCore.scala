...
object GenerateForSbtBloopCore extends BleepCodegenScript("GenerateForSbtBloopCore") {
  override def run(started: Started, commands: Commands, targets: List[Target], args: List[String]): Unit = {
    ...
    targets.foreach { target =>
      if (Set("sbt-bloop-core@jvm210", "sbt-bloop-core@jvm212").contains(target.project.value)) {
