...
case class BuildPaths(cwd: Path, bleepYamlFile: Path, variant: model.BuildVariant, wantedBleepVersion: Option[model.BleepVersion]) {
  ...
  /** `<workspace>/.bleep/builds/<variant>/history/` — one JSON transcript per completed compile/test request, written by the daemon, read by every client
    * (`bleep history show`, `bleep history diff`, the MCP tools). Workspace-local on purpose: it survives daemon restarts (the socket dir does not), and
    * removing a worktree removes its history. NEVER copied by copy-state or the remote cache — a transcript describes THIS workspace's runs.
    */
  lazy val historyDir: Path = workspaceVariantDir / "history"
