...
class OutputParserEdgeCaseTest extends AnyFunSuite with Matchers {
  ...
  test("TestResult: cancelled overrides passed") {
    val result = TestRunnerTypes.TestResult(100, 0, 0, 0, TestRunnerTypes.TerminationReason.Killed(bleep.bsp.KillReason.UserRequest))
    result.isSuccess shouldBe false
  }
  ...
  test("ScalaJs TestResult: same behavior as Native") {
    ...
    val cancelled = TestRunnerTypes.TestResult(100, 0, 0, 0, TestRunnerTypes.TerminationReason.Killed(bleep.bsp.KillReason.UserRequest))
