...
/** Every test in this class passes bleep's own classloader to `ScalaCollectionReflection`. That object builds collections inside the classloader a
  * `TestAdapter` brings. A defect in that object comes from the class that supplied a `Method` rather than from the classloader.
  */
