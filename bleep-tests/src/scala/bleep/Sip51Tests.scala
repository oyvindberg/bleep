...
class Sip51Tests extends AnyFunSuite with TripleEqualsSupport {
  ...
  def runTest(testName: String, yaml: String, files: Map[RelPath, String])(f: Started => Assertion): Assertion = {
  ...
  def getResolvedVersion(started: Started, projectName: model.CrossProjectName, org: String, name: String): Option[String] =
    for {
      resolution <- started.resolvedProjects(projectName).forceGet("test").resolution
  ...
  test("SIP-51: scala-library version must be at least scalaVersion (floor constraint)") {
    ...
    runTest(
      ...
      val resolvedLibrary = getResolvedVersion(started, projectName, "org.scala-lang", "scala-library")
  ...
  test("SIP-51: scala-library and scala-reflect must be at the same version") {
    ...
    runTest(
      ...
      val resolvedLibrary = getResolvedVersion(started, projectName, "org.scala-lang", "scala-library")
      val resolvedReflect = getResolvedVersion(started, projectName, "org.scala-lang", "scala-reflect")
