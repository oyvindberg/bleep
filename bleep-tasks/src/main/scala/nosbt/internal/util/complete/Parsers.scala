...
import java.io.File
import java.net.URI
import java.lang.Character.{getType, CURRENCY_SYMBOL, DASH_PUNCTUATION, MATH_SYMBOL, MODIFIER_SYMBOL, OTHER_PUNCTUATION, OTHER_SYMBOL}
...
trait Parsers {
  ...
  def isOpType(cat: Int) = cat match {
    case MATH_SYMBOL | OTHER_SYMBOL | DASH_PUNCTUATION | OTHER_PUNCTUATION | MODIFIER_SYMBOL | CURRENCY_SYMBOL => true;
    case _                                                                                                     => false
  }
  ...
  /** Parses a potentially quoted String value. The value may be verbatim quoted ([[StringVerbatim]]), quoted with interpreted escapes ([[StringEscapable]]), or
    * unquoted ([[NotQuoted]]).
    */
  lazy val StringBasic = StringVerbatim | StringEscapable | NotQuoted

  /** Parses a verbatim quoted String value, discarding the quotes in the result. This kind of quoted text starts with triple quotes `"""` and ends at the next
    * triple quotes and may contain any character in between.
    */
  ...
  /** Parses a single escape sequence into the represented Char. Escapes start with a backslash and are followed by `u` for a [[UnicodeEscape]] or by `b`, `t`,
    * `n`, `f`, `r`, `"`, `'`, `\` for standard escapes.
    */
  lazy val EscapeSequence: Parser[Char] =
    BackslashChar ~> ('b' ^^^ '\b' | 't' ^^^ '\t' | 'n' ^^^ '\n' | 'f' ^^^ '\f' | 'r' ^^^ '\r' |
      '\"' ^^^ '\"' | '\'' ^^^ '\'' | '\\' ^^^ '\\' | UnicodeEscape)

  /** Parses a single unicode escape sequence into the represented Char. A unicode escape begins with a backslash, followed by a `u` and 4 hexadecimal digits
  ...
  /** Applies `rep` zero or more times, separated by `sep`. The result is the (possibly empty) sequence of results from the multiple `rep` applications. The
    * `sep` results are discarded.
    */
  def repsep[T](rep: Parser[T], sep: Parser[_]): Parser[Seq[T]] =
    rep1sep(rep, sep) ?? Nil

  /** Applies `rep` one or more times, separated by `sep`. The result is the non-empty sequence of results from the multiple `rep` applications. The `sep`
  ...
  /** Defines a sequence parser where the parser used for each part depends on the previously parsed values. `p` is applied to the (possibly empty) sequence of
    * already parsed values to obtain the next parser to use. The parsers obtained in this way are separated by `sep`, whose result is discarded and only the
    * sequence of values from the parsers returned by `p` is used for the result.
    */
