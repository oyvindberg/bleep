...
object TokenCompletions {
  private[nosbt] abstract class Delegating extends TokenCompletions { outer =>
  ...
  private[nosbt] abstract class Fixed extends TokenCompletions { outer =>
  ...
  val default: TokenCompletions = mapDelegateCompletions { case (seen, level @ _, c) =>
    ctoken(seen, c.append)
  }
  ...
  def overrideDisplay(msg: String): TokenCompletions =
    mapDelegateCompletions { case (seen @ _, level @ _, c) =>
      tokenDisplay(display = msg, append = c.append)
    }
