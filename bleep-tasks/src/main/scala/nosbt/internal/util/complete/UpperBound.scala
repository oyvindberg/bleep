...
sealed trait UpperBound {
  ...
  /** If this bound is zero or Infinite, `decrement` returns this bound. Otherwise, this bound is finite and greater than zero and `decrement` returns the bound
    * that is one less than this bound.
    */
