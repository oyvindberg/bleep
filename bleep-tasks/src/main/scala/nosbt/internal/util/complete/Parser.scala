...
package nosbt.internal.util.complete

import Parser._

/** A String parser that provides semi-automatic tab completion. A successful parse results in a value of type `T`. The methods in this trait are what must be
  * implemented to define a new Parser implementation, but are not typically useful for common usage. Instead, most useful methods for combining smaller parsers
  * into larger parsers are implicitly added by the [[RichParser]] type.
  */
...
sealed trait RichParser[A] {
  ...
  /** Produces a Parser that applies the original Parser and then applies `next` (in order), discarding the result of `next`. (The arrow point in the direction
    * of the retained result.)
    */
  def <~[B](b: Parser[B]): Parser[A]

  /** Produces a Parser that applies the original Parser and then applies `next` (in order), discarding the result of the original parser. (The arrow point in
    * the direction of the retained result.)
    */
  ...
  /** Apply the original parser, but only succeed if `o` also succeeds. Note that `o` does not need to consume the same amount of input to satisfy this
    * condition.
    */
  ...
  /** @param exampleSource
    ...
    *   indicates whether completion examples should be checked for validity (against the given parser). Invalid examples will be filtered out and only valid
  ...
  /** Produces a Parser that filters the original parser. If 'f' is not true when applied to the output of the original parser, the Parser returned by this
    * method fails. The failure message is constructed by applying `msg` to the String that was successfully parsed by the original parser.
    */
...
object Parser extends ParserMain {
  ...
  final class Failure private[nosbt] (mkErrors: => Seq[String], val definitive: Boolean) extends Result[Nothing] {
    ...
    private[nosbt] def ++(f: Failure) = mkFailures(errors ++ f.errors)
  }
  ...
  def choiceParser[A, B](a: Parser[A], b: Parser[B]): Parser[Either[A, B]] =
    if (a.valid)
      if (b.valid) new HetParser(a, b) else a.map(Left.apply)
    else
      b.map(Right.apply)
...
trait ParserMain {
  ...
  implicit def richParser[A](a: Parser[A]): RichParser[A] = new RichParser[A] {
    def ~[B](b: Parser[B]): Parser[(A, B)] = seqParser(a, b)
    def ||[B](b: Parser[B]): Parser[Either[A, B]] = choiceParser(a, b)
    def |[B >: A](b: Parser[B]): Parser[B] = homParser(a, b)
    def ? : Parser[Option[A]] = opt(a)
    def * : Parser[Seq[A]] = zeroOrMore(a)
    def + : Parser[Seq[A]] = oneOrMore(a)
    def map[B](f: A => B): Parser[B] = mapParser(a, f)
    def id: Parser[A] = a

    def ^^^[B](value: B): Parser[B] = map(_ => value)
    def ??[B >: A](alt: B): Parser[B] = this.?.map(_ getOrElse alt)
    def <~[B](b: Parser[B]): Parser[A] = this.~(b).map { case av ~ _ => av }
    def ~>[B](b: Parser[B]): Parser[B] = this.~(b).map { case _ ~ bv => bv }
  ...
  /** Construct a parser that is valid, but has no valid result. This is used as a way to provide a definitive Failure when a parser doesn't match empty input.
    * For example, in `softFailure(...) | p`, if `p` doesn't match the empty sequence, the failure will come from the Parser constructed by the `softFailure`
    * method.
    */
  private[nosbt] def softFailure(msg: => String, definitive: Boolean = false): Parser[Nothing] =
  ...
  /** Defines a Parser that parses a single character only if the predicate `f` returns true for that character. If this parser fails, `label` is used as the
    * failure message.
    */
  ...
  implicit def literal(ch: Char): Parser[Char] = new ValidParser[Char] {
    def result = None
    def resultEmpty: Failure = mkFailure("Expected '" + ch + "'")
    def derive(c: Char) = if (c == ch) success(ch) else Invalid(resultEmpty)
  ...
  /** Convenience method to use when developing a parser. `parser` is applied to the input `str`. If `completions` is true, the available completions for the
    * input are displayed. Otherwise, the result of parsing is printed using the result's `toString` method. If parsing fails, the error message is displayed.
    *
  ...
  /** Convenience method to use when developing a parser. `parser` is applied to the input `str` and the result of parsing is printed using the result's
    * `toString` method. If parsing fails, the error message is displayed.
    */
  ...
  /** Convenience method to use when developing a parser. `parser` is applied to the input `str` and the available completions are displayed on separate lines.
  ...
  /** Applies parser `p` to input `s` and returns the completions at verbosity `level`. The interpretation of `level` is up to parser definitions, but 0 is the
    * default by convention, with increasing positive numbers corresponding to increasing verbosity. Typically no more than a few levels are defined.
    */
  ...
  def examples[A](a: Parser[A], completions: Set[String], check: Boolean = false): Parser[A] =
    examples(a, FixedSetExamples(completions), completions.size, check)
  ...
  /** @param a
    ...
    *   limits the number of examples that the source of examples should return. This can prevent lengthy pauses and avoids bad interactive user experience.
    * @param removeInvalidExamples
    *   indicates whether completion examples should be checked for validity (against the given parser). An exception is thrown if the example source contains
    *   no valid completion suggestions.
    * @tparam A
  ...
  /** Establishes delegate parser `t` as a single token of tab completion. When tab completion of part of this token is requested, the completions provided by
    * the delegate `t` or a later derivative are appended to the prefix String already seen by this parser.
    */
  def token[T](t: Parser[T]): Parser[T] = token(t, TokenCompletions.default)

  /** Establishes delegate parser `t` as a single token of tab completion. When tab completion of part of this token is requested, no completions are returned
    * if `hide` returns true for the current tab completion level. Otherwise, the completions provided by the delegate `t` or a later derivative are appended to
  ...
  /** Establishes delegate parser `t` as a single token of tab completion. When tab completion of part of this token is requested, `description` is displayed
    * for suggestions and no completions are ever performed.
    */
  def token[T](t: Parser[T], description: String): Parser[T] =
    token(t, TokenCompletions.displayOnly(description))

  /** Establishes delegate parser `t` as a single token of tab completion. When tab completion of part of this token is requested, `display` is used as the
    * printed suggestion, but the completions from the delegate parser `t` are used to complete if unambiguous.
    */
  ...
  private[nosbt] def mkToken[T](t: Parser[T], seen: String, complete: TokenCompletions): Parser[T] =
  ...
  def seq0[T](p: Seq[Parser[T]], errors: => Seq[String]): Parser[Seq[T]] = {
    val newErrors = p.collect { case Invalid(f) => () => f.errors }
    val valid = p.collect { case ok if !ok.isInstanceOf[Invalid] => ok }
...
private final class TrapAndFail[A](a: Parser[A]) extends ValidParser[A] {
  def result = try a.result
  catch { case _: Exception => None }
  def resultEmpty = try a.resultEmpty
  catch { case e: Exception => fail(e) }

  def derive(c: Char) = try trapAndFail(a derive c)
  catch {
    case e: Exception => Invalid(fail(e))
  }

  def completions(level: Int) = try a.completions(level)
  catch {
...
private final class ParserSeq[T](a: Seq[Parser[T]], errors: => Seq[String]) extends ValidParser[Seq[T]] {
  ...
  lazy val resultEmpty: Result[Seq[T]] = {
    val res = a.map(_.resultEmpty.toEither)
    val failures = res.collect { case Left(err) => err }
    val values = res.collect { case Right(value) => value }
...
/** This class wraps an existing parser (the delegate), and replaces the delegate's completions with examples from the given example source.
  *
  * This class asks the example source for a limited amount of examples (to prevent lengthy and expensive computations and large amounts of allocated data). It
  * then passes these examples on to the UI.
  *
  * @param delegate
  *   the parser to decorate with completion examples (i.e., completion of user input).
  * @param exampleSource
  *   the source from which this class will take examples (potentially filter them with the delegate parser), and pass them to the UI.
  * @param maxNumberOfExamples
  *   the maximum number of completions to read from the example source and pass to the UI. This limit prevents lengthy example generation and allocation of
...
private final class StringLiteral(str: String, start: Int) extends ValidParser[String] {
  ...
  def resultEmpty: Failure = mkFailure(failMsg)
  def result = None

  def derive(c: Char) =
    if (str.charAt(start) == c) stringLiteral(str, start + 1) else Invalid(resultEmpty)

  def completions(level: Int) = Completions.single(Completion.suggestion(str.substring(start)))
  override def toString = '"'.toString + str + '"'.toString
}

private final class CharacterClass(f: Char => Boolean, label: String) extends ValidParser[Char] {
  def result = None
  def resultEmpty: Failure = mkFailure("Expected " + label)
...
private final class Optional[T](delegate: Parser[T]) extends ValidParser[Option[T]] {
  def result = delegate.result map Some.apply
  def resultEmpty = Value(None)
  def derive(c: Char) = (delegate derive c).map(Some.apply)
...
private final class Repeat[T](
  ...
  private def repeatedParseEmpty: Result[List[T]] =
    if (min == 0)
      ...
      // forced determinism
      for (value <- repeated.resultEmpty) yield List.fill(min)(value)
