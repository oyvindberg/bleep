...
/** An enumeration defining the levels available for logging. A level includes all of the levels with id larger than its own id. For example, Warn (id=3)
  * includes Error (id=4).
  */
object Level extends Enumeration {
  ...
  private[nosbt] def unapply(s: String) = apply(s)
}
