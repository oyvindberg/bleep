...
sbt-vs-JUnit story is in [Test frameworks](/docs/appendix/test-frameworks).)
...
project pre-filter that skips compiling untagged projects), see [Test tags](/docs/usage/test-tags).
...
* Every run is recorded as a transcript; `bleep test --diff` shows only what changed
  versus a previous run. See [Run history & diffs](/docs/usage/run-history).
...
| `testTags` | map | Tag → suite class-name patterns for `--only-tag`/`--exclude-tag`. See [Test tags](/docs/usage/test-tags). |
