...
`.bleep/projects/<crossName>/generated-sources/annotations/`, a single directory per project per cross-id. All processors share this `-s` directory (per JSR 269). Output files separate naturally by Java package.
...
What a processor generates is a function of your sources and the processor coordinates, and both of those already go into the [remote cache](/docs/usage/remote-cache) digest. So the generated output itself is left out — the same treatment a Scala macro's expansion has always had, since that is never written anywhere to hash. It also stays out of what `--watch` and `bleep build invalidated` look at.

The practical effect: a processor that stamps a date into what it generates — MapStruct does, unless `mapstruct.suppressGeneratorTimestamp` is set — no longer moves the digest on every build and leaves the project, and everything downstream of it, permanently uncached. The option is still worth keeping on if you want byte-identical sources jars.

The flip side is a trust assumption worth knowing: bleep assumes a processor depends only on what it is given. One that reads a file, an environment variable or the network at compile time will not invalidate the cache when that changes. That is the same assumption bleep makes of macros, and the same reason a generator that genuinely needs outside input belongs in a [sourcegen script](/docs/concepts/sourcegen), whose output *is* keyed.
...
```
.bleep/projects/<crossName>/generated-sources/ksp/
...
[`bleep clean`](/docs/reference/cli/clean/) wipes them. Like
annotation-processor output, they are compiled but
[not part of the cache key](#not-part-of-the-cache-key).
