...
inspect your build through 18 structured tool calls, without
...
- **Compact by default.** Every tool returns a small JSON summary
  ...
  Full output is one extra call away, `bleep.status` for the
...
call; the response is small, a no-op.
...
- `verbose=true` on `bleep.compile` / `bleep.test` returns the full
  output inline. Use sparingly, it's much larger.
