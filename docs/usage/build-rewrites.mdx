---
title: Build rewrites
unlisted: true
---

# Build rewrites

> **Note:** This page documents an internal API. The script tutorials use the public
> `bleepscript.*` API; the rewrite hook below uses the internal `bleep.*` Scala API and
> may change without notice. The page is being held back from the navigation pending a
> public-API rewrite.
...
It takes a fully parsed and expanded build, and lets you take the complete set of projects and rewrite them to whatever you want.
...
You can also publish and/or package your app based on one of these builds.
