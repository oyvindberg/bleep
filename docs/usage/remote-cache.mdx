...
```bash
# Generate sources first — see below for why
bleep sourcegen
...
### The cache helps an empty workspace, not a warm one

`pull` skips any project whose classes directory already has something in it, **without looking at its digest**:

```
Remote cache pull: 0 pulled, 169 already compiled, 0 not cached
```

That is deliberate — a pull must never clobber a build you are in the middle of — but it has two consequences worth knowing before you plan around the cache:

- **A pull cannot repair state that is present but wrong.** Only `bleep clean` (or a fresh checkout) gets you back to a state a pull can populate.
- **Switching branches in place does not earn a cache hit.** The cache pays off where workspaces start empty: CI checkouts, fresh clones, and new git worktrees.

### Run `bleep sourcegen` before `pull` and `push`

Generated sources are part of a project's digest. In a workspace where sourcegen has not run yet, every project that consumes a generator — and everything downstream of it — hashes as *"no generated file"*, which is a key nothing was ever pushed under. The pull silently misses, and you compile what you could have downloaded.

On a 169-project build this was the difference between 148 and 167 projects restored, and between 53 s and 25 s of compiling afterwards. `bleep sourcegen` is cheap and idempotent; put it ahead of `remote-cache pull` in CI.

## How it works

Each project gets a SHA-256 **digest** computed from:
- The bleep version
- The build-level toolchain JVM — `jvm:` in `bleep.yaml`, name and index url (bump the JDK and every project's digest changes)
- Project configuration (dependencies, compiler flags, Scala version, platform, etc.)
- Source file contents (using git blob hashes for speed, with filesystem fallback), including what [sourcegen scripts](/docs/concepts/sourcegen) generate
...
- The output of annotation processors and KSP. It is a function of the sources and processor coordinates already listed above — the same way a macro's expansion is — so hashing it added nothing, except that a processor embedding a timestamp moved the digest on every build. See [annotation processing](/docs/usage/annotation-processing#not-part-of-the-cache-key) for the trust assumption that comes with it.
- [Build stamps](/docs/usage/stamps), and the `stamp:` declaration itself. A stamp is derived *from* the build — `project-digest` is this very digest — so hashing it would be circular, and a git-derived one would change the key on every commit. They never reach the compiler, which is what makes leaving them out safe.
...
2. the build's `jvm:` — the toolchain name and the index url it is looked
   up in, because a different JDK produces different class files and
   `jvm:` appears in no project's configuration,
3. the project's effective configuration as YAML — templates expanded,
   `publish` excluded because it doesn't affect compilation,
4. content hashes of every source file under the project's source
   directories,
5. content hashes of every resource file (resources are *not* stored in
   the archive, but they change the key),
6. the digests of all transitive project dependencies,
7. the digests of the projects backing its sourcegen scripts.
