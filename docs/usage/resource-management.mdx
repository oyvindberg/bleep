...
One knob: **`parallelism`** — how many operations bleep may run at once, compiles and forks together. It defaults to one per core, and everything else is derived from it (the daemon-wide compile ceiling is half of it, because a compile's scarce resource is the server's shared heap rather than a core).
