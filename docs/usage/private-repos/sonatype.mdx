...
Set the environment variables and run [`bleep publish sonatype`](/docs/reference/cli/publish/sonatype/). CI-agnostic — works with any CI.
...
Run [`bleep publish setup`](/docs/reference/cli/publish/setup/) to regenerate keys. Ensure `PGP_SECRET` is base64-encoded as a single line.
