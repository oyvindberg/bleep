...
The same resolver entry covers both fetching and publishing. Maven Central is still queried for everything Artifactory doesn't host, and **you cannot turn that off** — see [Mirror mode](#mirror-mode-artifactory-as-maven-central-proxy) below for what that means when your firewall blocks Maven Central.
...
If your firewall blocks Maven Central and your company fronts it through an Artifactory *virtual*
(or Nexus *group*) repository, declare that repository as a resolver:
...
It needs its own `authentications:` entry — bleep matches credentials by exact URI, so the virtual
repo is a different key from the release and snapshot repos even when the user and token are
identical:
...
Resolvers you declare are consulted **before** the defaults, so every artifact your mirror serves is
fetched from the mirror and Maven Central is never contacted for it.

### Maven Central cannot be removed

There is no `resolvers:` syntax that replaces the default list. `CoursierResolver.coursierRepos`
builds the repository list as `repos ++ constants.DefaultRepos`, and `constants.DefaultRepos` is
hardcoded to `~/.ivy2/local` followed by Maven Central. Whatever you write in `bleep.yaml`, those
two are appended.

If you need Central itself redirected rather than merely out-prioritised, use a Coursier mirror
file — that *does* work, and is documented under [Coursier mirrors do redirect
Central](/docs/usage/networking#coursier-mirrors-do-redirect-central).

Without one, leaving the entry in place is fine behind a mirror that serves everything: nothing ever
needs Central, so nothing ever asks it. It bites in two cases:

- an artifact your virtual repo does **not** serve (excluded by a policy, or not yet cached
  upstream). Bleep falls through to Maven Central, and on a network that blackholes it you get a
  connect timeout rather than a clean "not found".
- a fully air-gapped machine. See [Proxies, TLS & air-gapped
  networks](/docs/usage/networking) for what has to be pre-seeded and what still breaks.

If your firewall *rejects* rather than drops connections to `repo1.maven.org`, the fall-through
fails fast and costs you nothing. If it drops them, the per-request timeout is coursier's, not
bleep's, and bleep has no setting to shorten it.
