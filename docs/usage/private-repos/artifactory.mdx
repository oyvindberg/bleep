...
Auth lives in your **user config**, not in `bleep.yaml` (so credentials don't leak into source control). Path:

| OS | Path |
|----|------|
| macOS | `~/Library/Application Support/build.bleep/config.yaml` |
| Linux | `~/.config/bleep/config.yaml` (`$XDG_CONFIG_HOME/bleep/config.yaml` if set) |
| Windows | `%APPDATA%\build.bleep\config\config.yaml` |
...
Same idea on GitLab CI / Jenkins / Buildkite — write the config file from secrets, then run the publish command. macOS and Windows runners use the OS-specific paths from the table above.
