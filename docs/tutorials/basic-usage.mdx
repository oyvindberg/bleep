...
Every compile and test run is recorded into the workspace
(`.bleep/builds/normal/history/`, per worktree, the last 32 kept) — summaries
end with a `History: #N` line pointing at the transcript.
...
`--diff=<id>` pins a fixed baseline (say, the last green run) instead of the
previous run. The [run history & diffs guide](/docs/usage/run-history) tells
the whole story, with a demo video; [`bleep history`](/docs/reference/cli/history/)
is the reference.
...
- [Project globs](/docs/concepts/project-globs): the full
  vocabulary for naming projects and groups, plus how `cd`
  interacts with command scope.
- [Scripts](/docs/concepts/scripts): custom build logic as regular
  Java, Kotlin, or Scala programs you can debug.
- [Tab completions](/docs/tab-completions): install shell
