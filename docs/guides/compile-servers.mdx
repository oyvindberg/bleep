...
`bspReadTimeoutMinutes` is how long the server waits for the next message
from a client before dropping the connection, so a client that dies
without saying goodbye doesn't pin a server thread forever. It is a
per-read idle timeout, not a session limit: a long compile keeps the
socket quiet but the clock resets the moment the client speaks again, so
it can never interrupt a running build. Default 30; set `0` to wait
forever.
