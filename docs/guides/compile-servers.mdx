...
Compiles are bounded by **`parallelism`**, the one concurrency knob — see
[Memory & Parallelism](/docs/usage/resource-management#how-many-run-at-once).
The daemon-wide compile ceiling is half of it, because a compile's scarce
resource is the server's shared heap rather than a core: it allocates into
a heap every workspace on the daemon shares, so cores are the wrong proxy
for what it costs. On an 18-core machine, one compile per core drove 18
concurrent compiles into a single 12 GB heap until it died.

Test forks are not affected — they run out of process and are governed by
the fork-memory budget instead.

```bash
bleep config compile-server parallelism 4      # 4 operations at once, so 2 compiles
```
