 ...
 *
 * <p>This enables proper JUnit Platform lifecycle including LauncherSessionListener SPI,
 * which is required for frameworks like Quarkus that set up custom classloaders
 * (FacadeClassLoader) during session initialization.
 *
 * <p>Using openSession() instead of create() triggers:
 * - Quarkus's CustomLauncherInterceptor → FacadeClassLoader as TCCL
 * - Spring Boot's test context management
 * - Any other LauncherSessionListener implementations
 */
class JUnitPlatformRunner {
    ...
    void runSuite(
        ...
        send(TestProtocol.encodeLog("info", "Using JUnit Platform Launcher directly for: " + className));
        ...
        try {
            ...
            try (LauncherSession session = LauncherFactory.openSession()) {
                ...
                TestExecutionListener listener = new TestExecutionListener() {
                    ...
                    public void testPlanExecutionStarted(TestPlan testPlan) {
                        ...
                        send(TestProtocol.encodeLog("info",
                                "TestPlan started: " + count + " test(s) in plan, roots=" + testPlan.getRoots().size()));
                        for (TestIdentifier root : testPlan.getRoots()) {
                            send(TestProtocol.encodeLog("info", "  Root: " + root.getDisplayName() + " [" + root.getUniqueId() + "]"));
                            for (TestIdentifier child : testPlan.getChildren(root)) {
                                send(TestProtocol.encodeLog("info", "    Child: " + child.getDisplayName() + " isTest=" + child.isTest() + " [" + child.getType() + "]"));
                            }
                        }
                    }
                    ...
                    public void executionFinished(TestIdentifier testIdentifier, TestExecutionResult result) {
                        ...
                        send(TestProtocol.encodeTestFinished(
            ...
            send(TestProtocol.encodeSuiteDone(
                    className, passed[0], failed[0], skipped[0], ignored[0], durationMs));

        } catch (Throwable e) {
            send(TestProtocol.encodeLog("error",
