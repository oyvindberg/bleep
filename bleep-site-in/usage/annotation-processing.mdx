...
## MapStruct (separate annotation jar + processor jar)

MapStruct splits its API into a small `mapstruct` jar (annotations like `@Mapper`, runtime-needed) and a heavy `mapstruct-processor` jar (the processor — only needed at compile time). To keep the runtime classpath lean, list the annotations in `dependencies` and the processor in `annotationProcessors`:
...
### A caveat about JSR 269 service registration

Bleep relies on each processor jar having a `META-INF/services/javax.annotation.processing.Processor` entry. Most well-behaved processors do — MapStruct, Lombok, Immutables, Dagger.

A small number of processors don't, and instead rely on Gradle-specific or Maven-plugin-specific registration files. The most prominent example is **AutoValue 1.10.x**, whose `auto-value-*.jar` ships only `META-INF/gradle/incremental.annotation.processors` and `META-INF/maven/plugin.xml` — javac's standard ServiceLoader finds nothing. To use AutoValue with bleep today, pin the class name in `java.options`:
...
(`-processor` is one of the few flags bleep does not auto-wire — you can put it in `java.options` yourself.)
...
* **`-processor` (the comma-separated class-name list) is allowed in `java.options`.** Bleep never emits `-processor` itself; use it to pin a subset (e.g. one processor from a multi-processor jar) or to work around jars missing `META-INF/services` (see the AutoValue note above).
