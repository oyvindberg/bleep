...
object DependencyParser {
  ...
  /** Parses coordinates like org:name:version with attributes, like org:name:version,attr1=val1,attr2=val2 and a configuration, like org:name:version:config or
    ...
    val attrsOrErrors = rawAttrs
      .map { x =>
        if (x.contains(argSeparator))
          Left(
            s"'$argSeparator' is not allowed in attribute '$x' in '$input'. Please follow the format " +
      ...
      .getOrElse {
        ...
        validateAttributes(attrs, input, validAttrsKeys) match {
          ...
          case None =>
            ...
            parts0.flatMap { case (org, rawName, version, config, orgNameSep, withPlatformSuffix) =>
                  ...
                  val dep1 =
                    if (withPlatformSuffix)
                      dep0 match {
                        ...
                        case s: JavaOrScalaDependency.ScalaDependency => s.copy(withPlatformSuffix = true)
                      }
                    else
                      dep0
