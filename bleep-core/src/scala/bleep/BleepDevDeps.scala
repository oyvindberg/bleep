...
/** Resolves `build.bleep::*` artifacts from compiled class dirs instead of Coursier
  * when the version starts with "dev:".
  *
  * This is needed for forked sourcegen JVMs: they call `bootstrap.forScript()` which
  * re-resolves dependencies via Coursier, but `build.bleep::*:dev` artifacts aren't
  * published to any repository. Instead, we resolve them from the compiled class dirs
  * in the .bleep directory.
  *
  * Two resolution strategies:
  *   1. From buildDir: `<buildDir>/.bleep/builds/normal/.bloop/<name>/classes`
  *   2. From JVM classpath: scan `java.class.path` for matching entries (used by forked
  *      sourcegen JVMs where buildDir points to a consumer build, not bleep's own build)
  *
...
object BleepDevDeps {
  ...
  /** Hardcoded transitive `build.bleep::*` dependencies for each artifact.
    *
    * When we intercept a dev dep, we also need class dirs for its transitive
    * bleep dependencies since Coursier won't discover them (no POM to read).
    *
    * Verified by BleepDevDepsTest to match bleep's own build.
    */
  ...
  /** Compute the class dir path for a CrossProjectName within a build directory.
    *
    * Uses the same path convention as BuildPaths:
    * `<buildDir>/.bleep/builds/normal/.bloop/<name>/<crossId>/classes`
    */
  ...
  /** Resolve all class dirs for a `build.bleep::*` dep with a `dev:` version.
    *
    * First tries to resolve from the build dir encoded in the version. If those
    * class dirs don't exist (e.g., forked JVM running in a consumer build's temp dir),
    * falls back to scanning the JVM's own classpath for matching entries.
    *
    * Returns class dirs for the artifact itself plus all its transitive bleep deps.
    * Returns empty list if the artifact is not known or the version is not dev.
    */
  ...
  /** Scan the JVM's classpath for class dirs matching the given artifact and its transitive deps.
    *
    * SourceGenRunner launches forked JVMs with `-cp <classpath>` that includes all needed class
    * dirs. We match entries ending with `/<projectName>/classes` or `/<projectName>/<crossId>/classes`.
    */
